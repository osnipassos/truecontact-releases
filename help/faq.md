# TrueContact — Manual de Utilizador
...
3. [Sincronização de Contactos](#3-sincronização-de-contactos)
...
O **TrueContact** é um CRM pessoal automático para Mac e Windows. Liga-se ao iCloud, ao LinkedIn e ao WhatsApp para **sincronizar, enriquecer e organizar** os teus contactos — e envia mensagens de aniversário e de acompanhamento automaticamente, em segundo plano.

Ao contrário das soluções cloud (HubSpot, Salesforce, etc.), o TrueContact corre **100% no teu computador**. Não há subscrição, não há conta, não há servidor a guardar os teus dados.

### Para quem é?

- Profissionais de networking que querem manter contactos sempre atualizados
...
- Quem valoriza a **privacidade total** dos seus dados de contacto
...
2. Descarrega o ficheiro para o teu sistema operativo:
   - **macOS:** ficheiro `.dmg`
...
> **Importante (macOS):** Se o sistema mostrar o aviso "app de programador desconhecido", vai a **Definições do Sistema → Privacidade e Segurança** e clica em "Abrir na mesma". O TrueContact não está na App Store por opção — isso é intencional para manter a app gratuita.

### Primeira configuração

Após instalar, o fluxo recomendado é:

1. **Configurar o iCloud** — introduz o Apple ID e a App-Specific Password para importar os teus contactos
...
Toda a informação dos teus contactos — nomes, telefones, emails, notas, histórico de mensagens, fotos de perfil — é guardada numa base de dados **SQLite local**, numa pasta de dados da aplicação no teu disco rígido. Não existe nenhum servidor TrueContact a receber, processar ou armazenar esses dados.
...
O TrueContact apenas estabelece ligações à internet para:

| Ligação | Finalidade | Os teus dados passam pelos servidores TrueContact? |
|---|---|---|
| iCloud CardDAV | Sincronizar contactos com o iCloud | **Não** — ligação direta Apple |
| LinkedIn | Ler perfis públicos dos teus contactos | **Não** — só leitura, sem envio |
| WhatsApp Web | Enviar mensagens e ler fotos de perfil | **Não** — ligação direta WhatsApp |
| GitHub Releases | Verificar se há atualizações | **Não** — só verifica um número de versão |

**Nenhuma dessas ligações passa pelos servidores TrueContact** — porque esses servidores simplesmente não existem.
...
> **Aviso:** Nunca partilhes esta pasta com ninguém. Quem tiver acesso a estes ficheiros pode enviar mensagens em teu nome. Se suspeitas de comprometimento, vai a **WhatsApp → Definições → Aparelhos Ligados** e remove a sessão TrueContact.
...
Para gerar uma, vai a [appleid.apple.com](https://appleid.apple.com) → **Iniciar sessão e segurança → Palavras-passe específicas de app**.

---

## 3. Sincronização de Contactos

### Como o TrueContact une dados de várias fontes

O TrueContact funciona como um **Hub central**: o iCloud é a fonte de verdade principal, e o LinkedIn e o WhatsApp enriquecem esses contactos com dados adicionais.

O processo de sincronização funciona assim:

1. **Pull do iCloud** — Lê todos os contactos e grupos via CardDAV, preservando ETags para detetar alterações
2. **Enriquecimento LinkedIn** — Para contactos com perfil LinkedIn, visita automaticamente o perfil e importa cargo, empresa, foto e redes sociais
3. **Enriquecimento WhatsApp** — Para contactos com número de telefone, verifica a foto de perfil (se autorizado pelo contacto)
4. **Push para o iCloud** — Escreve as alterações de volta no iCloud, ficando disponíveis no iPhone, iPad e Mac

> **Regra fundamental:** O TrueContact **nunca apaga dados manuais**. Se adicionares uma nota ou um email a um contacto no iPhone, essa informação é preservada mesmo que o LinkedIn devolva um campo vazio.

### Normalização de números de telefone

Para unir corretamente um contacto do iCloud com a sua conta WhatsApp, o sistema precisa de comparar números de telefone. O mesmo número pode estar guardado de formas muito diferentes:
...
### Porque não vejo a foto de alguns contactos do WhatsApp?

Esta é uma das perguntas mais comuns. A resposta é simples: **o WhatsApp respeita as definições de privacidade de cada utilizador**.

Se um contacto configurou a sua foto de perfil para ser visível apenas para os seus próprios contactos (ou para ninguém), o WhatsApp não disponibiliza essa foto a nenhuma aplicação — incluindo o TrueContact. Quando isso acontece, o TrueContact mostra um **ícone de privacidade** em vez da foto, sinalizando que o contacto existe no WhatsApp mas optou por restringir a visibilidade da sua imagem.

**Isto é um comportamento correto, não um bug.** O TrueContact respeita as escolhas de privacidade dos teus contactos da mesma forma que o próprio WhatsApp faz.

> **Dica:** Se quiseres ver a foto real de um contacto específico, pede-lhe que abra as definições de privacidade do WhatsApp → Foto de perfil → "Todos os Meus Contactos".
...
4. Um **QR Code** aparecerá no ecrã — tens 60 segundos para o ler

**No teu telemóvel (iOS ou Android):**

1. Abre o **WhatsApp**
2. Vai a **Definições → Aparelhos Ligados**
3. Toca em **"Ligar um aparelho"**
4. Aponta a câmara para o QR Code no ecrã do computador
5. Aguarda a confirmação

> **Nota:** O TrueContact aparece como "Chrome em [nome do computador]" na lista de aparelhos ligados. O WhatsApp permite até 4 aparelhos em simultâneo.
...
O WhatsApp desliga aparelhos inativos por mais de 14 dias. Repete o processo de ligação.

**O WhatsApp diz "Número de aparelhos atingido":**
Remove um aparelho existente em **WhatsApp → Definições → Aparelhos Ligados** e tenta novamente.
...
A variável `{{nome_amigavel}}` é substituída pelo nome mais natural do contacto, seguindo esta ordem de prioridade:
...
> **Dica:** Para adicionar um apelido no iPhone, abre o contacto → "Editar" → "adicionar campo" → "Alcunha".

### Sorteio de mensagens e delay de digitação

**1. Sorteio aleatório:**  
Quando há múltiplos templates, o sistema escolhe um aleatoriamente. O mesmo contacto receberá uma mensagem diferente a cada aniversário.
...
2. Se encontrar uma versão nova, **descarrega-a em segundo plano** sem interromper o teu trabalho
3. Quando a transferência estiver completa, uma **notificação discreta** pergunta se queres instalar agora ou na próxima abertura
